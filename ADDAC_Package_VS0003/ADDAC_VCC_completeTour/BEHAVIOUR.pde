...
// MODES DEFINITION
  if(VCC.MODE==0){  //  MODE 0 has several test/debug functions for the available functions and modules
    if(VCC.SUBMODE==0){
     ...
     readAnalogsINA(5); // connected to input A -> (5) = reads 5 channels
      ...
      Quad.calcs(0,analogValsA[0], analogValsA[2], analogValsA[1],analogValsA[3],analogValsA[4]); // CARTESIAN / POLAR MODE (0=radius/angle; 1=x/y), RADIUS/X, ANGLE/Y, ANGLE MULTIPLIER, EXP/LOG, NA
  ...
  // NEXT SWITCH A POSITION --------------------------------------------------- if MODE A SWITCH is in Position 2
    ...
    VCC.WriteChannel(1, sin(millis()/1000.0f));
  // REMAINING SWITCH A POSITIONS
