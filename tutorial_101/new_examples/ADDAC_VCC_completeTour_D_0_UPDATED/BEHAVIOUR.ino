  ...
  if(VCC.MODE==0){  //  MODE 0 has several test/debug functions for the available functions and modules
    ...
    //
      ...
      for(int i=0; i<8;i++){
        ...
        VCC.WriteGatesStraight(B,i, int(random(2)));
      } 
      ...
      VCC.ReadGates(A,true); // Boolean Parameter sets inverting read setting  -> true:inverting, false:non-inverting
      for(int i=0; i<8;i++){
        #ifdef DEBUGgateINA
       ...
       /* if(VCC.gateValuesA[i]) {
          VCC.WriteChannel(i+1, addacMaxResolution);
          VCC.WriteGatesAstraight(i, 1);
        }else{
          VCC.WriteChannel(i+1, 0);
          VCC.WriteGatesAstraight(i, 0);
        }*/
  ...
  // NEXT SWITCH A POSITION --------------------------------------------------- if MODE A SWITCH is in Position 2
    ...
    if(VCC.SUBMODE == 0){
      ...
      for(int i=0;i<8;i++){
       ...
       ADSR1.adsrMode(adsrTrigger1, 0, 2000, //A
                                       0.5f, 2000, //D
                                       0.5f, 2000, //S
                                             2000); //R
       adsrTrigger1=false;
